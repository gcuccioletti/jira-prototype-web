...
class TimeAnalyzerConfig {
    constructor() {
        ...
        this.templatesKey = 'time_jql_templates';
    ...
    _autoSaveTemplateDraft() {
        ...
        this.setStorage('timeAnalyzerTemplateDraftName', name);
        this.setStorage('timeAnalyzerTemplateDraftQuery', query);
        this._updateStorageInfo();
    }
    ...
    _loadTemplateDraft() {
        ...
        if (nameInput && !nameInput.value) {
            nameInput.value = this.getStorage('timeAnalyzerTemplateDraftName') || '';
        }
        if (queryInput && !queryInput.value) {
            queryInput.value = this.getStorage('timeAnalyzerTemplateDraftQuery') || '';
        }
    }
     ...
     */
    _clearTemplateDraft() {
        this.setStorage('timeAnalyzerTemplateDraftName', '');
        this.setStorage('timeAnalyzerTemplateDraftQuery', '');
    }
    ...
    loadTemplate(index) {
        ...
        if (index >= 0 && index < templates.length) {
            ...
            window.location.href = `time-analyzer.html?template=${encodedQuery}#template=${encodedQuery}`;
        }
    }
    ...
    exportConfiguration() {
        ...
        link.href = url;
        link.download = `time-analyzer-config-${new Date().toISOString().split('T')[0]}.json`;
...
document.addEventListener('DOMContentLoaded', () => {
    window.configApp = new TimeAnalyzerConfig();
